...
def error_page_404(status, message, traceback, version):
    #page = PiUiPage(classHolder, '', None, None)
    #classHolder._handlers.new_page('ui', title='Novo Setup', page_obj=page)
    #page.postPush()
    #raise cherrypy.HTTPRedirect("", status=301)
    return urllib2.urlopen("http://10.0.0.5:9999").read()
